...
router.put('/like/:id', auth, async (req, res) => {
    try {
        ...
        if (liked) return res.status(400).json({ msg: 'Post already liked' });
...
router.put('/unlike/:id', auth, async (req, res) => {
    try {
        const post = await Post.findById(req.params.id);
        const unliked = post.likes.filter(like => like.user.toString() === req.user.id).length === 0;
        
        if (unliked) return res.status(400).json({ msg: 'Post has not yet been liked' });
